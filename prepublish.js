...
// Global variables.
const ORG = 'extra-map';
const ORIGINAL = 'Original module: [extra-map].';
...
function pkgMinify(o) {
  ...
  readme = readme.replace(/(\.<br>)/, ', exported as `Map`$1');
  readme = readme.replace(/(\.<br>)[\s\S]*?(\[!\[nodef\])/, `$1${EOL}${ORIGINAL}${EOL}${CDN}${EOL}<br>${EOL}${EOL}${EOL}$2`);
  readme = readme.replace(/extra-map\.min/g, 'extra-map');
  readme += `[unpkg]: https://unpkg.com/extra-map.min${EOL}`;
  readme += `[jsDelivr]: https://cdn.jsdelivr.net/npm/extra-map.min${EOL}`;
  pkg.name += '.min';
